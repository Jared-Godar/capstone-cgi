...
v_queryString_transformation = f"ALTER TABLE {v_athena_transformation_table}" + " ADD IF NOT EXISTS PARTITION\
...
try:
    ...
    v_job_details = 'Unable to create Athena partition within Insert Log function: ' + \
        v_process_step + ' for file: ' + file_name + ' - AWS Athena failure'
    v_error = str(e)
...
sys.exit(f'Exit - {v_job_details} - {v_error}')
...
def athena_query(athenaClient, queryString):
    return athenaClient.start_query_execution(QueryString=queryString, QueryExecutionContext={'Database': v_athena_db}, ResultConfiguration={'OutputLocation': f's3://{v_s3_bucket}/{v_athena_results_folder}'}, WorkGroup=v_athena_workgroup)
